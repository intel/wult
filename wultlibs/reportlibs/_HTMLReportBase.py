...
class HTMLReportBase:
    ...
    def generate(self):
        ...
        # 'pinfos' are generated by the 'PlotsBuilder' which is not aware of the report output
        # directory. Therefore this sets 'pinfo["path"]' to accurately reflect where this plot
        # can be found in the final output.
        for pinfos in self._pinfos.values():
            for pinfo in pinfos:
                ppath = self._plotsdir.joinpath(pinfo["fname"])
                pinfo["path"] = str(ppath.relative_to(self.outdir))
