...
from wultlibs.helperlibs import Trivial, _ParserBase
    ...
    for pkginfo in packages.values():
        for coreinfo in pkginfo["cores"].values():
            ...
            if "totals" not in coreinfo:
                coreinfo["totals"] = OrderedDict()
            for key, val in sums.items():
                coreinfo["totals"][key] = calc_total(sums[key]["sum"], sums[key]["count"], key)
        ...
        if "totals" not in pkginfo:
            pkginfo["totals"] = OrderedDict()
        for key, val in sums.items():
            pkginfo["totals"][key] = calc_total(sums[key]["sum"], sums[key]["count"], key)
...
def _construct_the_result(totals, cpus, nontable):
    ...
    result["packages"] = packages = OrderedDict()
