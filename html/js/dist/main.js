...
`,fe=document.createElement("style");fe.textContent=be.toString(),document.head.append(fe);var me,$e=qt`
  ${ge}
...
`,fs=0,ms=class extends te{constructor(){super(...arguments),this.localize=new Se(this),this.componentId="sl-tab-"+ ++fs,this.panel="",this.active=!1,this.closable=!1,this.disabled=!1}focus(t){this.tab.focus(t)}blur(){this.tab.blur()}handleCloseClick(){Oe(this,"sl-close")}render(){return this.id=this.id||this.componentId,wt`
      <div
        part="base"
    ...
    `}};ms.styles=gs,We([Je(".tab")],ms.prototype,"tab",2),We([Fe({reflect:!0})],ms.prototype,"panel",2),We([Fe({type:Boolean,reflect:!0})],ms.prototype,"active",2),We([Fe({type:Boolean})],ms.prototype,"closable",2),We([Fe({type:Boolean,reflect:!0})],ms.prototype,"disabled",2),We([Fe()],ms.prototype,"lang",2),ms=We([Ve("sl-tab")],ms);var $s=qt`
  ${ge}
...
`,_s=0,ys=class extends te{constructor(){super(...arguments),this.componentId="sl-tab-panel-"+ ++_s,this.name="",this.active=!1}connectedCallback(){super.connectedCallback(),this.id=this.id||this.componentId}render(){return this.style.display=this.active?"block":"none",wt`
  ...
  `;connectedCallback(){super.connectedCallback(),this.paths=this.info.ppaths,this.smrystblpath=this.info.smrytblpath}visibleTemplate(){return z`
            <br>
    ...
    `;static properties={tabFile:{type:String},tabs:{type:Object,attribute:!1}};connectedCallback(){super.connectedCallback(),fetch(this.tabFile).then((t=>t.json())).then((t=>{this.tabs=t}))}tabTemplate(t){return t.tabs?z`
          <sl-tab-group>
            ${t.tabs.map((t=>z`
             <sl-tab slot="nav" panel="${t.id}">${t.label}</sl-tab>
             <sl-tab-panel id="${t.id}" name="${t.id}">${this.tabTemplate(t)}</sl-tab-panel>
             `))}
          </sl-tab-group>
        `:z`
      <wult-metric-tab tabname="${t.id}" .info="${t.mdata}"></wult-metric-tab>
      `}render(){return this.tabs?z`
            <sl-tab-group>
              ${this.tabs.map((t=>z`
                  <sl-tab slot="nav" panel="${t.id}">${t.label}</sl-tab>
                  <sl-tab-panel name="${t.id}">${this.tabTemplate(t)}</sl-tab-panel>
                `))}
            </sl-tab-group>
