...
def _generate_diff(outdir, paths, fp):
    """
    Helper function for '_add_fpreviews()'. Generates an HTML diff of the files at 'paths' with path
    'fp' in a "diffs" sub-directory. Returns the path of the HTML diff relative to 'outdir'.
    """
    ...
    diff_path = (outdir / "diffs" / fp).with_suffix('.html')
    try:
    ...
    return diff_path.relative_to(outdir)
    ...
    def add_fpreviews(self, base_paths, files):
        ...
        for name, fp in files.items():
            ...
            for reportid, base_path in base_paths.items():
                ...
                if self._outdir not in src_path.parents:
